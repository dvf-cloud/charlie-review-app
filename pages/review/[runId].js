...
export default function ReviewPage() {
  ...
  async function fetchRecord() {
    try {
      ...
      let nd = null;
      try {
        let ndRaw = rec.fields.nutrition_data;
        if (typeof ndRaw === 'string') ndRaw = JSON.parse(ndRaw);
        // Handle {error, raw} format — Agent 2 returned markdown-wrapped JSON
        if (ndRaw && ndRaw.error && ndRaw.raw) {
          const cleaned = ndRaw.raw.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
          ndRaw = JSON.parse(cleaned);
        }
        // Handle array format [{name, carbs_per_100g}] → convert to object keyed by name
        if (Array.isArray(ndRaw)) {
          const obj = {};
          ndRaw.forEach(item => { if (item.name) obj[item.name.toLowerCase()] = item; });
          ndRaw = obj;
        } else if (ndRaw && ndRaw.ingredients && Array.isArray(ndRaw.ingredients)) {
          const obj = {};
          ndRaw.ingredients.forEach(item => { if (item.name) obj[item.name.toLowerCase()] = item; });
          ndRaw = obj;
        }
        nd = ndRaw;
      } catch(e) { nd = null; }
  ...
  const isModeB = !isModeA && (modeRaw.toLowerCase().includes('mode b') || modeRaw.toLowerCase().includes('baked') || modeRaw.toLowerCase().includes('weight-first'));
  ...
  function lookupNutrition(name, nd) {
    if (!nd) return null;
    const n = name.toLowerCase();
    // Helper — normalize result to {carbs_per_100g} format
    const normalize = (val) => {
      if (!val) return null;
      if (typeof val.carbs_per_100g === 'number') return val;
      return null;
    };
    // Manual overrides — map ingredient name to nd key
    const overrides = {
      ...
      'rote bete': 'rote bete', 'olivenöl': 'olivenöl', 'öl': 'öl',
      'zitronensaft': 'zitronensaft', 'zitrone': 'zitrone', 'essig': 'essig',
      'schinkenwürfeli': 'schinken',
    };
    const overrideKey = Object.keys(overrides).find(k => n.includes(k));
    if (overrideKey) return normalize(nd[overrides[overrideKey]]) || null;
    // Fuzzy — keys are lowercase names
    let key = Object.keys(nd).find(k => n.includes(k));
    if (!key) key = Object.keys(nd).find(k => k.includes(n.split(' ')[0]));
    if (!key) {
      const stem = n.replace(/^bio-/, '').replace(/^tk-/, '').replace(/^kalte\s/, '').split(' ')[0];
      key = Object.keys(nd).find(k => k.includes(stem) || stem.includes(k));
    }
    return key ? normalize(nd[key]) : null;
  }
